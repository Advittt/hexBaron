...
namespace HexBaronCS
{
    class Program
    {
        ...
        public static void SetUpDefaultGame(out Player player1, out Player player2, out HexGrid grid)
        {
            ...
            player1 = new Player(a, 0, 10, 20, 5);
            player2 = new Player(b, 1, 10, 10, 5);
            grid.SetUpGridTerrain(t);
        ...
        public static bool CheckUpgradeCommandFormat(List<string> items)
        {
            ...
            if (items.Count == 3)
            {
                if (items[1].ToUpper() != "LESS" && items[1].ToUpper() != "PBDS" && items[1].ToUpper() != "GARDENER")
                    return false;
                try
                {
    ...
    class LESSPiece : Piece
    {
        ...
        public int Planton(string terrain)
        {
            if (terrain == "$")
            {
                return 1;
    ...
    class GARDENERPiece : Piece
    {
        public GARDENERPiece(bool player1)
            : base(player1)
        {
            pieceType = "G";
            VPValue = 1;
        ...
        public int Plant(string terrain)
        {
            if (terrain == " ")
            {
                return 1;
    ...
    class HexGrid
    {
        ...
        public void AddPiece(bool belongsToPlayer1, string typeOfPiece, int location)
        {
            ...
            else if (typeOfPiece == "GARDENER")
            {
                newPiece = new GARDENERPiece(belongsToPlayer1);
            }
            else
            {
        ...
        private int ExecuteUpgradeCommand(List<string> items, int lumberAvailable)
        {
            ...
            if (!CheckPieceAndTileAreValid(tileToUse) || lumberAvailable < 5 || !(items[1] == "pbds" || items[1] == "less" || items[1] == "gardener"))
            {
                return -1;
            }
            else
            {
                ...
                else if (items[1] == "gardener")
                {
                    thePiece = new GARDENERPiece(player1Turn);
                }
                else
                {
